...
def draw_(t):
    ...
    stroke(0)
    for i in range(50):
        pos1 = path.trace(i / 51.0)
        d1 = path.get_direction(i / 51.0)
        v1 = d1.cross(d1.getRotatedX(HALF_PI))
        v1.normalizeTo(r)

        pos2 = path.trace((i + 1) / 51.0)
        d2 = path.get_direction((i + 1) / 51.0)
        ...
        for j in range(51):
            p1 = v1.getRotatedAroundAxis(d1, j * TWO_PI / 50.0)
            vert1 = p1.add(pos1)
            vertex(vert1.x(), vert1.y(), vert1.z())
            p2 = v2.getRotatedAroundAxis(d2, j * TWO_PI / 50.0)
            vert2 = p2.add(pos2)
            vertex(vert2.x(), vert2.y(), vert2.z())
        endShape(CLOSE)

    translate(pos2.x(), pos2.y(), pos2.z())
    d1 = path.get_direction(0)
    d2 = path.get_direction(1)
    rotateX(d2.headingYZ() - d1.headingYZ())
    rotateY(d2.headingXZ() - d1.headingXZ())
    rotateZ(-d2.headingXY() + d1.headingXY())
